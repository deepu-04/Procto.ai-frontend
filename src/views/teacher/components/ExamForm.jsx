...
const ExamForm = ({ formik, isDark: propIsDark }) => {
  ...
  // Dynamic Questions Array (Strictly Coding)
  ...
  const canProceed = () => {
    if (activeStep === 0) {
      if (!values.examName || !values.duration || !values.liveDate || !values.deadDate) return false;
    ...
    if (activeStep === 1) {
      for (const q of questions) {
        if (!q.questionText || !q.description) return false;
        if (q.testCases.length === 0) return false;
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      ...
      /* ================= GET EXAM ID SAFELY ================= */
      const examId = examRes?.examId || examRes?.data?.examId || examRes?.exam?.examId || examRes?.exam?._id;
      ...
      for (const [index, q] of questions.entries()) {
        try {
          const cleanedTestCases = q.testCases.filter((tc) => tc.input && tc.output);

          const questionPayload = {
            examId,
            type: 'coding', // Force coding designation
            section: 'coding', // Force coding designation
          ...
          console.log(`Creating question ${index + 1}:`, questionPayload);
          await createQuestion(questionPayload).unwrap();
        } catch (qErr) {
          console.error(`Error creating question ${index + 1}:`, qErr);
          toast.error(`Failed to upload Question ${index + 1}. Check console for details.`);
        }
      }
      ...
      const errorMessage = err?.data?.message || err?.response?.data?.message || err?.message || 'Failed to create exam';
  ...
  const purpleButtonStyle = {
    ...
    '&.Mui-disabled': {
      bgcolor: isDark ? 'rgba(139, 92, 246, 0.3)' : '#C4B5FD',
      color: isDark ? 'rgba(255,255,255,0.5)' : 'white',
    },
  };
          ...
          >
            ...
            {activeStep === 0 && (
              <Stack spacing={0}>
                ...
                <Typography variant="body2" sx={{ color: isDark ? '#94A3B8' : 'textSecondary', mb: 4 }}>
                  Set the core details, duration, and target audience for your new coding assessment.
                </Typography>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12}>
                    <CustomTextField
                      label="Exam Name"
                      name="examName"
                      placeholder="e.g., Mid-Term Advanced Data Structures"
                ...
                <Box mt={4}>
                  ...
                  <FormControl component="fieldset">
                    <RadioGroup row value={audienceType} onChange={(e) => setAudienceType(e.target.value)}>
                      <FormControlLabel
                        value="all"
                        control={<Radio sx={{ color: isDark ? '#A78BFA' : '#7C3AED', '&.Mui-checked': { color: isDark ? '#A78BFA' : '#7C3AED' } }} />}
              ...
              <Stack spacing={0}>
                <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={1}>
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ color: isDark ? '#94A3B8' : 'textSecondary', mb: 4 }}>
                      Add coding problems to your assessment.
                    </Typography>
                  </Box>
                  <Button
                    variant="outlined"
                      ...
                      <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}>
                          ...
                          {q.image && (
                            ...
                            >
                              <img src={q.image} alt="Preview" style={{ maxHeight: 150, borderRadius: 8 }} />
                            </Box>
                          )}
                        </Grid>
                          ...
                          {q.testCases.map((tc, tcIndex) => (
                            ...
                            >
                              ...
                              <CustomTextField
                                ...
                                fullWidth
                                value={tc.output}
                                onChange={(e) => updateTestCase(q.id, tcIndex, 'output', e.target.value)}
                                sx={customInputStyle}
                              />
                              <FormControlLabel
                                control={
                                  <Switch
                                    checked={tc.isHidden}
                                    onChange={(e) => updateTestCase(q.id, tcIndex, 'isHidden', e.target.checked)}
                                    color="secondary"
                                  />
                                }
                ...
                <TipsBox
                  ...
                  tips={[
                    'Provide at least one hidden test case to prevent hardcoding.',
                    'Use images to clarify complex architectural or mathematical problems.',
                    'Provide detailed instructions and examples in the description.',
                  ]}
                />
              </Stack>
            )}
            ...
            {activeStep === 2 && (
              <Stack spacing={0}>
                ...
                <Typography variant="body2" sx={{ color: isDark ? '#94A3B8' : 'textSecondary', mb: 4 }}>
                  Verify your coding exam details before sending out the invites.
                </Typography>
                ...
                >
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={6}>
                      <Typography variant="body2" sx={{ color: isDark ? '#E2E8F0' : 'inherit' }}>
                        <b style={{ color: isDark ? '#94A3B8' : 'inherit' }}>Live Window:</b>{' '}
                        {new Date(values.liveDate).toLocaleString()}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography variant="body2" sx={{ color: isDark ? '#E2E8F0' : 'inherit' }}>
                        <b style={{ color: isDark ? '#94A3B8' : 'inherit' }}>Audience:</b>{' '}
                        {audienceType === 'all' ? 'All Students' : `${targetEmails.length} Specific Emails`}
                      </Typography>
                    </Grid>
                  </Grid>
                </Box>
                ...
                <Box
                  sx={{
                    ...
                    '&::-webkit-scrollbar-thumb': { bgcolor: isDark ? 'rgba(255,255,255,0.2)' : '#CBD5E1', borderRadius: 3 },
                  }}
                >
                  {questions.map((q, i) => (
                    <Box
                    ...
                    >
                      ...
                      <Chip
                        size="small"
                        label={`${q.testCases.length} Test Cases`}
                        sx={{
                          bgcolor: isDark ? 'rgba(124, 58, 237, 0.2)' : '#F3E8FF',
                          color: isDark ? '#C4B5FD' : '#7C3AED',
                          fontWeight: 'bold',
                ...
                <Box display="flex" gap={2}>
                  ...
                  <Button
                    ...
                    startIcon={creatingExam || creatingQuestion ? <CircularProgress size={20} color="inherit" /> : null}
